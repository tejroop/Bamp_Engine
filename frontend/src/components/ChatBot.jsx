...
function simulatePrice(price, market = 'HK', marketing = 50000, gap = 0) {
  ...
  const attachRate = Math.max(0, Math.min(90, logisticRate + mktDelta + compDelta));

  const demandMultiplier = Math.exp(cfg.elasticity * Math.log(price / cfg.optimalPrice));
  ...
  const accessoryRevenue = accessoryUnits * cfg.accPrice;
  ...
  return { price, attachRate, predictedDemand, accessoryUnits, totalRevenue, revenueChange, mattressRevenue, accessoryRevenue, optimalPrice: cfg.optimalPrice };
}
...
const INTENTS = [
  {
    ...
    handler: (query, market) => {
      ...
      return {
        text: `At a mattress price of €${price} for ${market === 'HK' ? 'Hong Kong' : 'Taiwan'}, the model predicts:\n\n` +
          `• **Attachment Rate**: ${sim.attachRate.toFixed(1)}% (β=${d.attachment_beta})\n` +
    ...
    handler: (query, market) => {
      ...
      return {
        text: `**Hong Kong vs Taiwan — Head-to-Head Comparison:**\n\n` +
          ...
          `| Attach. Slope (β) | ${hk.attachment_beta} | ${tw.attachment_beta} |\n` +
          `| Elasticity | ${hk.elasticity} (inelastic) | ${tw.elasticity} (elastic) |\n` +
          `| Gap β (Ch.6) | ${hk.gap_beta} | ${tw.gap_beta} |\n` +
          `| Revenue Optimum | €${hk.optimal_price} | Entry €${tw.optimal_price_entry} / Mid €${tw.optimal_price_mid} / Prem €${tw.optimal_price_premium} |\n\n` +
